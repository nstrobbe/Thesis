\chapter{The need for physics beyond the Standard Model
\label{chap:beyond_standard_model}}
\chaptermark{The need for BSM physics}
...
the remaining 69\% of the energy budget of the universe is \textbf{dark energy}, and drives the
accelerating expansion of the universe. The Planck results also indicate that this dark energy is
consistent with the assumption of a cosmological constant. The SM again does not provide an
explanation for this; in fact, any attempt to compute this cosmological constant in terms of vacuum
energy leads to a mismatch of around 100 orders of magnitude. 
...
the Higgs potential mass parameter $\mu^2$ up to very large values.
This is often referred to as the hierarchy problem.
...
compelling theoretical reason to assume that baryon and lepton number are conserved quantities. 
  
  \item Why is the $\mu^2$ parameter in the Higgs potential ($\mu^2 \Phi^\dagger \Phi + \lambda
(\Phi^\dagger \Phi)^2$) negative? Within the SM this is an assumption, without
...
Observed particle masses are modelled as a combination of the bare, tree-level mass, and all
radiative corrections from additional loop diagrams. The loop momenta are cut off at the scale where
we believe the theory to be no longer valid, in this case the Planck scale. 
...
There is nothing in principle wrong with this degree of fine tuning, however, many
view it as unnatural, and many models of
...
In an attempt to address some of the aforementioned open questions, numerous models of
...
the corrections to the Higgs mass are suppressed by two loops with respect to the cutoff scale, and
as such the hierarchy problem only appears around a scale of 10\TeV, compared to 1\TeV for the SM. 
