...
def Plot1DPAS(hdictlist,outputdir="plots",outfile=0,legdict=0,cname="canvas"
    ...
    maxi = 0
    for hdict in hdictlist:
        h_ = hdict["histogram"] # Get the histogram
        h_.Sumw2() # need to put this otherwise errors in ratio plot are wrong
        if scale == "Yes":
            sf = h_.Integral(0,h_.GetNbinsX()+1)
            h_.Scale(scalefactor/sf)
        if scale == "Width":
            h_.Scale(scalefactor,"width")
        if scale == "Yes":
            h_.GetYaxis().SetTitle("A.U.")
        else:
            h_.GetYaxis().SetTitle("Events / bin")
        if h_.GetMaximum() > maxi:
            maxi = h_.GetMaximum()
